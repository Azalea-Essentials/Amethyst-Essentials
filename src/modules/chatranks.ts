...
class ChatranksModule {
    ...
    call(msg: ChatSendBeforeEvent) {
        ...
        if(!ranks.length) {
            if(playerManager.isAdmin(msg.sender)) ranks.push(configuration.modules.chatranks.defaultAdminRank)
            else ranks.push(configuration.modules.chatranks.defaultMemberRank)
        }
        for(const player of world.getPlayers()) {
