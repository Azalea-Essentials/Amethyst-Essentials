...
import { addItemResponse, chestguis } from "apis/ChestGUIMaker"
...
let ui = {
    ...
    open(player: Player, options: UIOptions) {
        ...
        modal.show(player).then(res=>{
            if(res.canceled) return root.open(player);
            let errors = [];
            if(!res.formValues[0]) errors.push("No Icon ID defined!");
            // @ts-ignore
            if(!iconManager.isValidIcon(res.formValues[0])) errors.push("Invalid Icon");
            if(!res.formValues[1]) errors.push("No Name Defined");
            if(!res.formValues[3]) errors.push("Please define command");
            if(options.openType != 1 && chest.icons.find(_=>_.row == res.formValues[3] && _.column == res.formValues[4])) errors.push("Icon Already Exists In Slot");
            if(errors.length) {
                ui.open(player, {
                    ...options,
                    error: errors.join(', ')
