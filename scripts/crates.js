import { world, Player, system, EnchantmentTypes } from "@minecraft/server";
import { ActionFormData, MessageFormData, ModalFormData } from "@minecraft/server-ui";
import { any, armor, bow, crossbow, fishingrod, sword, axe, pickaxe, hoe, shovel } from "./modules/enchantments";
import { set1, set2, set3, set4, set5, set6, set7, set8, set9, set10, rewardnames, keynames } from "./rewards";
                    ...
                    for (let player of world.getPlayers({ name: playername })) { //way2 - check player
                        try {
                            ...
                            if (typeof commands == `string`) {
                                player.runCommandAsync(commands);
                            }
...
world.beforeEvents.itemUse.subscribe(create => {
    system.run(() => {
...
world.afterEvents.entityHitEntity.subscribe((crate) => {
    let player = crate.damagingEntity;
    let crateName;
    if (crate.hitEntity?.typeId === `azalea:crate` && player instanceof Player) {
        if (!crate.hitEntity.hasTag(`named`) && !crate.hitEntity.hasTag(`noname`))
            return;
